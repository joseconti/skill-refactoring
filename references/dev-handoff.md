...
- `ARCHITECTURE.md` — **[Planner]** new architecture, layers, patterns, decisions and trade-offs (Phase 3).
- `SECURITY.md` — **[Planner]** the platform's security checklist the development AI must follow (Phase 4).
- `LIFECYCLE.md` — **[Planner]** idempotent install, update, deactivate and uninstall with the optional cleanup, Danger Zone (Phase 5).
- `TESTING.md` — **[Planner -> Dev AI]** test strategy defined by the planning assistant (Phase 6); the development AI implements it and keeps the doc updated. Covers the full stack: unit/integration tests (PHPUnit + WP test suite for WP/Woo; the equivalent for other platforms), static analysis (PHPStan/Psalm), coding standards (PHPCS with WordPress/WooCommerce sniffs), and the golden tests for the locked contracts — all green in blocking CI, with their config files (`phpunit.xml.dist`, `phpstan.neon`, `phpcs.xml`) and a CI workflow.
...
1. Set up a test environment and verify each part in it before considering it done.
...
9. Write and keep the automated test stack GREEN before declaring anything done: unit/integration tests (PHPUnit + WP test suite for WP/Woo; the equivalent for other platforms), static analysis (PHPStan/Psalm), coding standards (PHPCS with the WordPress/WooCommerce sniffs for WP), and the golden tests for the locked contracts. Run them in blocking CI. Never declare a sprint complete with any check red.
...
> Load the software's main directory `<name/path>`. First READ the spec files I provide in `refactoring/docs/` — `CODE-MAP.md`, `DATA-MODEL.md`, `ARCHITECTURE.md`, `SECURITY.md`, `LIFECYCLE.md`, `TESTING.md` — and the mandatory rules block; treat them as the source of truth and do not rewrite their decisions. Then CREATE your own `PLAN.md` (split into short sprints) and follow it, and as you build create and keep updated `HOOKS.md`, `CLASSES.md`, `USAGE.md`, `API.md`/`REST-API.md` and a `CONTINUE-PHASE-N.md` at the end of each sprint. Comply with the sacred rules (security, escaping, sanitization, platform styles) and NEVER invent: if something is unclear, ask. Set up a test environment and test each phase (WordPress Playground only if this is WP/Woo; otherwise the equivalent local setup for this platform — never block because "the Playground can't be created"). At the end of each sprint, summarize the state.
...
> Explicitly declare that the work is done. Generate a return prompt for the planning assistant that includes an index of all the documentation created and a summary of the final state, so it can do a full review at a glance. If there is a playground or anything testable locally, explain to the person step by step how to test it.
